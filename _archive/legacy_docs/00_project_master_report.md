...
> **본 문서의 목적**: 데이터 수집 → 통합 → 전처리 → 목표변수 설계 → 모델링 → 성능 검증 → 기존 모형과의 비교 → 실서비스 포털화 및 모델 심화 검증·재학습까지, 이 저장소에서 진행된 전체 작업을 하나의 문서에서 처음부터 끝까지 순서대로 확인할 수 있도록 정리한 종합 리포트입니다. 개별 상세 보고서(`docs/step01_to_06_*.md` ~ `docs/step29_*.md`)의 총괄 목차이자 요약본 역할을 하며, 각 절 하단에 원본 상세 문서를 링크해 두었습니다.
>
> 프로젝트 전체 이력은 세 단계로 나뉩니다. **① 모델링 단계**(Step 1~13): 원천 데이터로부터 부도 예측 LightGBM 모델을 만들고 검증하는 과정. **② 포털 단계**(Step 14~27): 그 모델을 실사용자가 쓰는 FastAPI+DuckDB+React 웹 서비스로 만들고, 서비스 곳곳의 "기존 모형과의 비교" 로직이 진짜 실측 데이터를 쓰도록 다시 검증·수정한 과정. **③ 고도화 및 재재점 단계**(Step 28~29): 교수님·전문가 피드백을 반영해 7대 심화 검증(과적합·다중공선성·워크포워드 CV·벤치마크 등)을 수행하고, 프로덕션 모델 재학습 및 194만 행 DB 전수 재채점과 운영 런북 수립을 마친 과정.
...
10. [모델 고도화 및 프로덕션 재학습·재채점 (Step 28~29)](#10-모델-고도화-및-프로덕션-재학습재채점-step-2829)
11. [한계 및 후속 과제](#11-한계-및-후속-과제)
12. [부록: 전체 Step 타임라인](#12-부록-전체-step-타임라인)
...
- **핵심 결론 (Executive Summary)**: 신규 AI 모형은 Out-of-Time 검증에서 **AUROC 0.9005**(2026-07-04 정규화 재학습 반영치, [step29](step29_production_model_retrain_and_rescore.md))를 기록했으며, 은행이 실제로 보유한 기존 모형 실측 벤치마크(RZVL_POD, AUROC 0.823) 대비 우위를 보였다. 더 중요한 것은 **은행 내부 조기경보등급이 "정상(A등급)"으로 분류했던 차주 중 실제 부도가 발생한 175개사**를 추적한 결과, 그중 **63.4%(111개사)는 부도 순간까지 내부등급이 끝내 A로 유지**되어 완전한 사각지대였고, 나머지 **36.6%(64개사)**에 대해서는 신규 모형이 내부등급 하향 대비 **평균 6.8개월 더 빠르게** 위험을 경고했다는 점이다.
...
| 지표 | Train | Valid | 해석 |
|---|:---:|:---:|---|
| **ROC-AUC** | 0.9958 | **0.9005** | 무작위 분류(0.5) 대비 압도적, 과거 내부지표(0.77대) 대비 우수 |
| **Gini Index** | 99.2 | **80.1** | 통상 60 이상이면 우수 — 80대는 탁월 |
| **K-S Statistic** | 94.3 | **66.6** | 기준치 40을 크게 상회, 역전 없는 분리력 |
| **PSI** | - | **0.1357** | 기준치 0.25 이하 — 시간 경과에도 점수 분포 안정 |

> 2026-07-04 정규화 파라미터 재학습([step28](step28_model_validation_and_benchmarking.md)/[step29](step29_production_model_retrain_and_rescore.md)) 반영치. 이전 수치(0.9011/80.2/67.2/0.1247)는 early stopping이 Valid를 재사용해 낙관적으로 편향돼 있었다.
...
> §8.2~8.3의 63.4%/36.6%/6.8개월 수치는 `eda_pipeline/step11~12`의 별도 오프라인 분석 결과로, 2026-07-04 재학습된 모델 기준으로는 아직 재검증하지 않았다(범위 밖, [step29](step29_production_model_retrain_and_rescore.md) 참고). §8.4의 벤 다이어그램·리드타임은 실시간 SQL 집계라 재채점 직후 자동으로 새 값을 반영하며, 아래는 그 새 값이다.
...
| 구분 | 건수 |
|---|---:|
| 둘 다 포착 (ERM + 내부등급 모두 사전 경고) | 403건 |
| ERM만 포착 (내부등급은 끝내 미포착 또는 사후 반영) | 547건 |
| 내부등급만 포착 | 6건 |
| 둘 다 미포착 | 16건 |
| **합계** | **972건** |

내부등급 단독 포착 사례가 소수(6건) 발생하지만, ERM 계열(both+erm_only=950건)이 내부등급 계열(both+internal_only=409건)을 압도적으로 상회하는 구조는 그대로 유지된다. 리드타임 평균은 **11.3개월**(좌측 절단 309건 제외, 유효 94건 기준) — 2026-07-04 재학습·재채점 후의 수치([step29](step29_production_model_retrain_and_rescore.md) §4)이며, 재학습 전에는 각각 409/556/0/7건, 평균 12.9개월(좌측절단 313건 제외, 유효 96건)이었다.
...
## 10. 모델 고도화 및 프로덕션 재학습·재채점 (Model Enhancement & Production Rescoring)

프로젝트 완료 후, 교수님 및 외부 전문가 피드백을 수용하여 모델링 방법론의 정당성을 철저히 재검증하고(Step 28), 그 검증 결론을 반영해 실제 운영 중인 LightGBM 모델 재학습 및 194만 행 데이터베이스 패널 전체 재채점을 완수했다(Step 29).

### 10.1 교수님 피드백 대응 5대 심화 검증 및 2대 추가 분석 (Step 28)

모델링의 과학적 타당성을 입증하기 위해 다음과 같은 총 7대 항목의 심층 진단 및 벤치마크 실험을 수행했다.

| 검증 항목 | 핵심 문제 제기 | 검증 방법 및 결과 |
|---|---|---|
| **① 과적합(Overfitting) 진단 및 정규화** | Train(0.9958)과 Valid(0.9011) 간의 AUC 격차가 다소 큼. Early Stopping이 Valid 셋을 참조해 누수(Leakage) 발생 가능성 | Train 마지막 3개월을 전용 **Dev 셋(2023.10~12)으로 분리**하여 Early Stopping 대상에서 Valid를 완전히 배제. 추가로 `num_leaves=15`, `min_child_samples=100`, `reg_alpha/lambda=1.0` 등 강한 정규화 파라미터를 적용한 결과, Train AUC는 0.96대로 낮아지면서도 순수 Hold-out Valid AUC는 오히려 상승하는 진정한 일반화 성능 향상 확인 |
| **② SHAP 안정성 및 중복 변수 검증** | 트리 모델 특성상 변수 간 중요도(SHAP) 순위가 쉽게 흔들리거나 중복 변수들끼리 기여도를 나눠 가질 우려 | Train vs Valid 및 3회 부트스트랩 샘플링 간 상위 30/50개 변수의 **스피어만 상관계수가 >0.98**로 극도로 높은 안정성 입증. 상위 30개 변수 간 상관관계 분석 결과 VIF 다이어트가 중복을 이미 성공적으로 필터링했음을 확인 |
| **③ 변수 수 축소 (Top N Ablation)** | 실무 운용을 위해 상위 20~30개 변수만으로 극단적 축소가 가능한지 여부 | Top 20/30/50/80/100 변수 셋 모델 학습 결과, Top 20/30은 AUC 및 재현율 저하가 뚜렷하게 발생함. **현재의 Lean(80개 변수) 모델이 예측력 손실 없이 경량화를 달성한 최적의 균형점**임을 수리적으로 증명 |
| **④ 워크포워드 시계열 교차검증** | 66개월 전체 구간에서 단일 시점 분할(OOT)만 사용할 경우 특정 경제 시점에 편향되거나 시간 경과에 따른 성능 열화(Drift) 미탐지 위험 | 관측 구간을 6개월 단위 롤링으로 이동하는 **14-Fold Walk-Forward CV** 수행. 테스트 폴드 AUC가 평균 0.90~0.92 수준으로 매우 강건하게 유지됨을 확인. 최근 구간의 표면적 지표 하락은 우측 절단(Right-censoring)에 의한 자연적 현상임을 규명 |
| **⑤ 타 머신러닝 모델 벤치마크** | LightGBM 외에 로지스틱 회귀, 랜덤 포레스트, XGBoost 대비 우위 입증 필요 | **Logistic Regression(AUC 0.814)**, **Random Forest(AUC 0.887)**, **XGBoost(AUC 0.898)**와 정면 비교. 비선형성 및 결측치(`use_missing=true`)가 많은 SME 재무 데이터 특성상 LightGBM(0.901)이 압도적 속도와 최고의 예측 성능을 동시에 달성함을 실증 |
| **⑥ [추가검증] 순수 3-Way Split 검증** | Train/Valid/Test를 완전히 독립된 시간축(2021~23 / 2024 / 2025~26H1)으로 분리할 때의 진짜 일반화 성능 | 한번 학습 후 18개월 이상 갱신하지 않은 모델의 순수 Test AUC는 0.880으로 열화됨을 확인. 그러나 동일 구간에서 Walk-Forward 재학습을 적용할 경우 ~0.906으로 성능이 완벽히 복원됨을 실증하여, **분기/반기 단위 주기적 재학습 정책의 필수성**을 수립 |
| **⑦ [추가검증] 228개 변수 전수 다중공선성(VIF) 진단** | 기존 VIF 다이어트가 Top-100 변수에 한정되어 있어 전체 변수 셋 내 잠재적 다중공선성 식별 필요 | 전수 검사 결과, 원천 API 지표와 3개월 이동평균(`_ma3`)이 동반 편입된 거시/원자재 변수군에서 고공선성 발생 식별. 심지어 현재 서비스 중인 Lean(80) 모델 내에서도 환율·유가 ma3 등 **3개 변수의 VIF가 무한대에 도달**함을 규명하여 Step 29 제거 대상 확정 |

### 10.2 검증 결과 반영 프로덕션 모델 재학습 및 전사 194만 행 재채점 (Step 29)

Step 28의 심화 검증과 진단 결과를 연구 보고서에 그치지 않고, 실제 서비스 운영 DB(`portal.duckdb`) 및 실시간 API 추론 엔진에 end-to-end로 완벽히 적용했다.

- **① 프로덕션 LightGBM 모델 파라미터 교체 및 재학습**:
  - **Full 모델(230개 변수)**: 정규화 파라미터(`num_leaves=15`, `min_child_samples=100`, `reg_alpha=1.0`, `reg_lambda=1.0`) 및 Dev 셋 Early Stopping 적용 → 정직한 순수 Hold-out Valid AUC **0.9005**, Gini **80.1**, K-S **66.6**, PSI **0.1357**을 달성(기존 Baseline 0.8905 대비 향상).
  - **Lean 모델(80 → 77개 변수)**: Step 28에서 규명된 무한대 VIF 중복 변수 3개(`EUR_KRW_ma3`, `USD_KRW_ma3`, `WTI_USD_ma3`)를 삭제하여 **77개 변수로 경량화**했음에도 Valid AUC **0.9023**이라는 최고의 압축 효율을 달성.
- **② 194만 행(`corporate_panel`) In-Place 전수 재채점**:
  - 원천 CSV 파일 재로딩 없이, `portal.duckdb` 내 1,944,418행의 피처를 직접 읽어 신규 모델로 재학습 스코어를 계산하는 전용 파이프라인(`database/rescore_full_model.py`)을 구축하여 `PROB_FULL`, `Z_SCORE`, `Z_GRADE`를 100% 갱신 완료.
- **③ Z-Score 정규화 및 16단계 등급 컷오프(`PROB_CUTOFFS`) 갱신**:
  - 기존의 하드코딩된 정규화 근사식 대신, 실제 재학습 확률 분포의 실측 평균(`-3.4560`)과 표준편차(`2.0123`) 기반 공식으로 교체.
  - 전사 등급 맵핑 모듈(`backend/grade_mapping.py`)의 16단계 확률 컷오프 상수를 신규 분포에 맞춰 자동 재산출 및 갱신.
- **④ E2E 서비스 정합성 검증 및 운영 런북 수립**:
  - **포털 UI/API 검증**: 글로벌 대시보드 KPI, 예측 벤 다이어그램(403/547/6/16, 리드타임 11.3개월), 차주 상세, SHAP 기여도, 매크로 시뮬레이션 등 포털 전 기능이 신규 재채점 스코어를 완벽히 반영해 작동함을 검증.
  - **운영 런북(Runbook)**: 모델의 시간적 감쇠를 방지하기 위해 분기 단위 PSI 모니터링 및 재학습 파이프라인(`step23_retrain_production_models.py` → `rescore_full_model.py`) 실행 매뉴얼을 수립.

**상세**: [step28_model_validation_and_benchmarking.md](step28_model_validation_and_benchmarking.md), [step29_production_model_retrain_and_rescore.md](step29_production_model_retrain_and_rescore.md)
...
| 28 | 과적합·SHAP 정제·다중공선성·워크포워드 CV·타 모델 비교 검증 (교수님 피드백 대응) | [step28](step28_model_validation_and_benchmarking.md) |
| 29 | 검증 결과 반영 재학습(정규화·Dev 분리·Lean 중복변수 제거) + 194만 행 전체 재채점 + 등급 컷오프 재계산 | [step29](step29_production_model_retrain_and_rescore.md) |
