...
## 3. 🎯 전방 12개월 부도 발생여부(`TARGET_12M`, `TTD <= 12`) 타겟 엔지니어링의 수학적·도메인적 명분
AI 신용평가 모형 설계에서 심사위원들이 가장 날카롭게 검증하는 요소는 바로 **"타겟 변수를 어떻게 정의했는가(Target Engineering)"**입니다. 저희 팀은 특정 기준월(`BASE_YM`) 시점에서 단기(1~3개월)가 아닌 **"향후 12개월 이내에 당행 부도(`DSH_DT`)가 발생하는가?"**를 이진 분류 타겟(`TARGET_12M`)으로 설정했습니다.

### 수학적 타겟 정의 (Mathematical Formulation)
...
### ⭐ 전방 12개월 타겟(`TARGET_12M`)을 선택한 3대 금융 도메인 핵심 명분
...
## 4. 🛡️ 발표 심사위원 심층 면접 대비 Q&A 대본 (Top 5)
