...
## 4. ⚖️ [핵심 대조 분석] AS-IS(0.~3. 기존 모델링 방법) vs TO-BE(4. 개선 모델링 방법) 비교 매트릭스
본 프로젝트가 기존 신용평가 모형의 통계적·도메인적 한계를 어떻게 진단하고, SME 4.0 ERM 모형으로 돌파했는지를 보여주는 6대 핵심 비교표입니다.
...
### 💡 AS-IS 모형의 시간축 과적합(Time-dummy Overfitting) 교훈과 TO-BE 돌파 전략
* **기존 모형의 한계 규명**: 기존 0.~3. 단계 모델링(`two_track_scoring_model.pkl`)은 거시경제 172개 변수를 과도하게 투입한 결과, 트리 알고리즘이 특정 시점의 금리·환율 숫자 자체를 시간 이름표처럼 통째로 암기하는 **'시간축 과적합(Time-dummy Overfitting)'**을 겪었습니다(Train AUC 0.86 → OOT AUC 0.58 폭락). 이를 해결하기 위해 투트랙 분리 구조를 썼으나 OOT AUC 0.6405에 머물렀습니다.
* **TO-BE 모형의 완벽한 돌파**: 저희 팀은 194만 행의 대규모 중소기업 전수 패널을 확보하고, 전방 12개월 부도 타겟(`TARGET_12M`)으로 설계를 혁신했습니다. 나아가 다중공선성을 일으키는 변수들을 VIF 10.0 기준으로 과감히 자동 제거(VIF Diet)하여 단일 엔드투엔드 파이프라인만으로 **미래 미공개 검증(OOT) AUROC 0.902+ 및 +12.9개월 선제 조기경보**라는 역사적 돌파를 이루어냈습니다!
