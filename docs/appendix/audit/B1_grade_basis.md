...
검증일: 2026-09-03 · **재검증: 2026-09-04 (D8 164피처 재학습·재채점 후)** · 대상 DB `database/portal_v2.duckdb` (read_only) · `corporate_panel` 948,214행
...
로그 `logs/B1_grade_basis.log` · `logs/track1_grade.log` · `logs/track1_rescore.log`

> **2026-09-04 재검증 결과:** OBV 죽은 피처 5개 제외로 D8 이 169→164 피처가 되어
> 등급 컷오프·μ/σ·등급별 부도율을 전면 재산출했다. **5단계 G1~G5 단조성은 Valid·전
> 구간 모두 유지**(0.0629 → 0.201 → 0.829 → 4.097 → 19.78% / 전 구간 0.0185 → …
> → 45.21%), 16단계 순위 불일치도 여전히 **0 / 948,214**. 결론과 구조는 불변이고
> Z-사다리 상세표(§1)·μ/σ·등급별 부도율(§4)을 전부 새 값으로 갱신했다.
...
z_mu    = -4.243331831837524    # D8 164피처 재학습 후 재산출 (구: -4.186387031107627)
z_sigma =  2.887110220603069    # 구: 2.837346856919158
```
...
|--:|--:|--:|--:|--:|---|
| — | — | 0.000800 | −7.1304 | **−1.0000** | *(G2 시작)* |
| 1 | 0.400 | 0.007425 | −4.8954 | −0.2259 | AA+ |
| — | — | 0.014156 | −4.2433 | **+0.0000** | *(G3 시작)* |
| 2 | 0.500 | 0.016231 | −4.1045 | +0.0481 | AA0 |
| 3 | 0.575 | 0.028471 | −3.5300 | +0.2471 | AA− |
| 4 | 0.650 | 0.049563 | −2.9537 | +0.4467 | A+ |
| 5 | 0.715 | 0.080782 | −2.4318 | +0.6275 | A0 |
| 6 | 0.770 | 0.122864 | −1.9656 | +0.7889 | A− |
| 7 | 0.820 | 0.183574 | −1.4923 | +0.9529 | BBB+ |
| — | — | 0.204855 | −1.3562 | **+1.0000** | *(G4 시작)* |
| 8 | 0.865 | 0.266850 | −1.0107 | +1.1197 | BBB0 |
| 9 | 0.900 | 0.360283 | −0.5741 | +1.2709 | BBB− |
| 10 | 0.928 | 0.464672 | −0.1415 | +1.4207 | BB+ |
| 11 | 0.950 | 0.569771 | +0.2809 | +1.5671 | BB0 |
| 12 | 0.967 | 0.662688 | +0.6753 | +1.7037 | BB− |
| 13 | 0.980 | 0.751831 | +1.1084 | +1.8537 | B+ |
| — | — | 0.822136 | +1.5309 | **+2.0000** | *(G5 시작)* |
| 14 | 0.989 | 0.826084 | +1.5581 | +2.0094 | B0 |
| 15 | 0.996 | 0.901460 | +2.2136 | +2.2365 | CCC |

> 굵은 Z 값(−1 / 0 / +1 / +2)은 5단계 경계이고 그 사이 값이 16단계 경계다.
> **하나의 정렬된 사다리**이며 서로 교차하지 않는다.
>
> 16단계 사다리는 Z ∈ [−0.23, +2.24] 구간만 분해한다 — 그 아래(G1 전체와 G2 대부분)는
...
|---|--:|--:|--:|--:|--:|
| AAA | 178,801 | 230,831 | 0 | 0 | 0 |
| AA+ | 0 | 79,371 | 17,226 | 0 | 0 |
| AA0 | 0 | 0 | 71,245 | 0 | 0 |
| AA− | 0 | 0 | 70,852 | 0 | 0 |
| A+ | 0 | 0 | 59,092 | 0 | 0 |
| A0 | 0 | 0 | 49,240 | 0 | 0 |
| A− | 0 | 0 | 43,965 | 0 | 0 |
| BBB+ | 0 | 0 | 11,895 | 26,495 | 0 |
| BBB0 | 0 | 0 | 0 | 28,607 | 0 |
| BBB− | 0 | 0 | 0 | 21,935 | 0 |
| BB+ | 0 | 0 | 0 | 16,054 | 0 |
| BB0 | 0 | 0 | 0 | 11,829 | 0 |
...
> G5 는 `B+`(397) · `B0`(6,118) · `CCC`(6,849) 세 밴드에만 존재하고, 그 셋은 16단계
...
|---|--:|--:|--:|--:|--:|:-:|
| G1 | 52,431 | 52,431 | 33 | 33 | 0.0629 | 일치 |
| G2 | 98,074 | 98,074 | 197 | 197 | 0.2009 | 일치 |
| G3 | 109,863 | 109,863 | 911 | 911 | 0.8292 | 일치 |
| G4 | 48,381 | 48,381 | 1,982 | 1,982 | 4.0966 | 일치 |
| G5 | 3,585 | 3,585 | 709 | 709 | 19.7768 | 일치 |
...
| GRADE | n | 점유율% | 부도 | 부도율% |
|---|--:|--:|--:|--:|
| G1 | 178,801 | 18.86 | 33 | 0.0185 |
| G2 | 310,202 | 32.71 | 200 | 0.0645 |
| G3 | 323,515 | 34.12 | 951 | 0.2940 |
| G4 | 122,332 | 12.90 | 2,588 | 2.1156 |
| **G5** | 13,364 | 1.41 | 6,042 | **45.2110** |

전 구간 기저율 1.0350% (9,814 / 948,214). **단조성 확보.**

> ★ 전 구간 G5 부도율 45.21% 는 Valid 값 19.78% 와 다르다. 같은 컷오프인데 값이 다른
> 이유는 **구간 기저율 차이**다 — Train/Dev(< 202301) 0.7876% vs Valid(≥ 202301) 1.2254%.
> 어느 하나가 틀린 것이 아니라 **집계 모집단이 다르다.** 문서에 등급별 부도율을 쓸 때는
> 반드시 구간을 병기해야 한다. 홀드아웃 성능을 말하는 자리에서는 **Valid 값(19.78%)이
> 정본**이다.

같은 이유로 16단계의 전 구간 실현 점유율도 설계 분위와 다르다 — `AAA` 는 설계 40%
(Valid 분위)인데 전 구간에서는 **43.2%** 를 차지한다. Train/Dev 구간의 예측 PD 가
전반적으로 낮아 40th-Valid-percentile 아래로 더 많이 몰리기 때문이다.

### 단조성에 관한 정확한 서술 (2026-09-04 재산출 반영)

- **5단계 `GRADE`: Valid · 전 구간 모두 단조 확보.** `z_cutoffs_adjusted = false` —
  `step40` 의 분위 대체 폴백(L120-131)이 발동하지 않았다. 등폭 컷오프 그대로다.
- **16단계 `GRADE16`: `monotone_g16 = true`.** 164피처 재산출 후 **Valid·전 구간 모두
  역전 0쌍**이 됐다. 초기 169피처 시점에 있던 `AA0` > `AA−` 역전 1쌍(표본 노이즈)이
  사라졌다. 다만 인접 밴드 부도 건수 차이가 여전히 작아(예: 전 구간 `AA0` 0.1418%
  vs `AA−` 0.2202%, 부도 101건 vs 156건) **다른 표본·다른 세대에서는 다시 뒤집힐 수
  있는 여유폭이다.** 16단계는 표기용 체계로 서술하고, 리스크 판정은 5단계만 쓴다.
